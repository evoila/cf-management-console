...
import com.github.cfc.api.repositories.OrganizationRepository;
import com.github.cfc.api.repositories.SpaceRepository;
...
public class OrganizationRepositoryTest {
    ...
    public void testDeleteByIdShouldFailWhenOrganizationIsNotDeleted() throws IOException {
        ...
        when(restTemplate.exchange(eq("/api/v2/organizations/123?inline-relations-depth=1"), eq(HttpMethod.GET), isA(HttpEntity.class), isA(ParameterizedTypeReference.class))).thenReturn(new ResponseEntity(organizationResponse, HttpStatus.OK));
        when(restTemplate.exchange(eq("/api/v2/organizations/123"), eq(HttpMethod.DELETE), isA(HttpEntity.class), isA(ParameterizedTypeReference.class))).thenReturn(new ResponseEntity(HttpStatus.NO_CONTENT));
